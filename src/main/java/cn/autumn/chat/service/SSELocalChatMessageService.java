...
@AllArgsConstructor
@Slf4j
public class SSELocalChatMessageService {
    ...
    private void sendUserMessage(String chatId, String message, long msgId) {
        messageService.sendOrderedMessage(new LocalChatMessageResp(chatId, msgId, ChatMessageRole.USER, message));
        messageService.sendOrderedMessage(new LocalChatMessageResp(chatId, msgId, "", true, false));
    }
    ...
    private void handleAiResult(String chatId, Long msgId, String resultMsg, StringBuffer aiResult) {
        ...
        messageService.sendOrderedMessage(new LocalChatMessageResp(chatId, msgId, resultMsg, false));
    }

    private void handleAiCompletion(String chatId, StringBuffer aiResult, String key, Long msgId) {
        stopAnswering(key);
        messageService.sendOrderedMessage(new LocalChatMessageResp(chatId, msgId, "", true));
        log.info("ai回答已完成");
    }

    private void handleAiError(String chatId, Long msgId) {
        messageService.sendOrderedMessage(new LocalChatMessageResp(chatId, msgId, AI_RESULT_PREFIX, true));
    }
